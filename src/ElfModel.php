...
class ElfModel{
  ...
  public function __construct($table_name = null){require_once(__DIR__.'/config.php');if($table_name)$this->table_name = $table_name;}
  ...
  public function create($row){
    ...
    return $this->dbInstance($GLOBALS['elf_mysql'], 'master')->lastInsertId();
  }
  ...
  public function execute($sql, $params = array(), $readonly = false){
    ...
    if($readonly && !empty($GLOBALS['elf_mysql']['MYSQL_SLAVE'])){
      $slave_key = array_rand($GLOBALS['elf_mysql']['MYSQL_SLAVE']);
      $sth = $this->dbInstance($GLOBALS['elf_mysql']['MYSQL_SLAVE'][$slave_key], 'slave_'.$slave_key)->prepare($sql);
    }else{
      $sth = $this->dbInstance($GLOBALS['elf_mysql'], 'master')->prepare($sql);
    }
  ...
  public function dbInstance($db_config, $db_config_key, $force_replace = false){
    if($force_replace || empty($GLOBALS['elf_mysql_instances'][$db_config_key])){
      try {
        if(!class_exists("\PDO") || !in_array("mysql",\PDO::getAvailableDrivers(), true)){
          err('Database Err: \PDO or \PDO_MYSQL doesn\'t exist!');
        }
        $GLOBALS['elf_mysql_instances'][$db_config_key] = new \PDO('mysql:dbname='.$db_config['MYSQL_DB'].';host='.$db_config['MYSQL_HOST'].';port='.$db_config['MYSQL_PORT'], $db_config['MYSQL_USER'], $db_config['MYSQL_PASS'], array(\PDO::MYSQL_ATTR_INIT_COMMAND=>'SET NAMES \''.$db_config['MYSQL_CHARSET'].'\''));
      }catch(\PDOException $e){err('Database Err: '.$e->getMessage());}
    }
    return $GLOBALS['elf_mysql_instances'][$db_config_key];
  }
