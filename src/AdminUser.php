...
class AdminUser extends Admin
{
    ...
    public function getUserInfo($userid)
    {
        ...
        */
        if ($user_info) {
            ...
            $role_model = new ElfModel('sys_role');
            $role = $role_model->find(array("id" => $role_info["role_id"]));
    ...
    */
    public function addUser($role_id, $user)
    {
        ...
        /*管理员信息保存*/
        $user['password'] = $this->mymd5_4($user['password']);
        $this->create($user);
    ...
    */
    public function updateUser($id, $roleid, $new_data)
    {
        if (@$new_data['password']) {
            $new_data['password'] = $this->mymd5_4($new_data['password']);
        }
